...
N, S, Nj, wij, Hij, Vr, Rrsi, hw = scenario_setting(scenario_number=8) # insert sceneraio number
...
for k in range(iterations):
    ...
    for ix in np.arange(x_min,x_max+res,res):
        for iy in np.arange(y_min,y_max+res,res):
            ...
            for robots in range(N):
                distances[robots] = (np.sqrt(np.square(ix - current_x[robots]) + np.square(iy - current_y[robots]))- weight[robots]) #proposed
    ...
    if k == 0:
        Hg, Hp, Ht, Hr, Hgen = cost(N,locations,[current_x,current_x],Vr_dash,weight,hw) 
    else:
...
save_list_to_csv(proposed_cost, 'c:\\Users\\swara\\Documents\\GitHub\\MRS_Unified_Coverage\\robotarium_python_simulator_MRS_UGA\\rps\\mrs_unified_coverage\\csv\\s8\\proposedCostB4.csv')
# save_list_to_csv(cumulative_distance, 'c:\\Users\\swara\\Documents\\GitHub\\MRS_Unified_Coverage\\robotarium_python_simulator_MRS_UGA\\rps\\mrs_unified_coverage\\csv\\s6\\proposedCostB2.csv')

...
for k in range(iterations):
    ...
    if k > 0:
        for plot in hull_figHandles:
            plot.remove()      
    hull_figHandles =[]
    dvx = [[] for i in range(N)]
    dvy = [[] for i in range(N)]
    ...
    for s in range(len(S)):
        ...
        for ix in np.arange(x_min,x_max+res,res):
            for iy in np.arange(y_min,y_max+res,res):
                ...
                distances = np.zeros(len(Nj[s]))
                for robots in Nj[s]:
                    distances[robots- Nj[s][0]] = np.sqrt(np.square(ix - current_x[robots-1]) + np.square(iy - current_y[robots-1]))
        ...
        for r, robots in  enumerate(Nj[s]):
            ...
            hullHandle, = (robo.axes.plot(xss, yss, color=CM[s], linewidth=3))
            ...
            if not w_v[r] == 0:
                c_x = c_v[r][0] / w_v[r]
                c_y = c_v[r][1] / w_v[r] 
                si_velocities[:, robots-1] = 1 * [(c_x - current_x[robots-1][0]), (c_y - current_y[robots-1][0] )]
        
        Hg, Hp, Ht, Hr, Hgen = cost(len(Nj[s]),locations,[[current_x[i-1] for i in Nj[s]],[current_y[i-1] for i in Nj[s]]],Vr,weight,hw,Rr)
        Hg_ += float(Hg[0])
        Hp_ += float(Hp[0])
        Ht_ += float(Ht[0])
        Hr_ += float(Hr[0])
    ...
    locational_cost.append(float(Hg_))
    health_cost.append(float(Hp_))
    mobility_cost.append(float(Ht_))
    range_cost.append(float(Hr_))
    proposed_cost.append(float(Hgen_))
    iteration_.append(k)
    ...
    robo.step()
    robo.axes.scatter(x[0,:], x[1,:], s=5, color= [CM[i] for i in range(N)], marker='x') # plotting trajectory
    ...
    if all(d <=1e-3 for d in diff):
        if convergence_flag:
            converged_T = k
            print("Converged")
            title_ = f'Baseline-1, Scenario = {scenario_number}, Converged at t = {converged_T}'
            print(title_)
            robo.axes.text(-1.5,y_max+0.1, title_, fontsize=12, fontweight='bold') #Plot title
            plt.savefig(f'./plot/plot/baseline1_s{scenario_number}.png')
...
# Save to CSV
df.to_csv(f'./plot/cost/baseline1_cost_all{scenario_number}.csv', index=False)  # `index=False` to avoid adding row indices

...
for k in range(iterations):
    ...
    if all(d <=1e-3 for d in diff):
        if convergence_flag:
            converged_T = k
            print("Converged")
            title_ = f'Baseline-3, Scenario = {scenario_number}, Converged at t = {converged_T}'
            print(title_)
            robo.axes.text(-1.5,y_max+0.1, title_, fontsize=12, fontweight='bold') #Plot title
            plt.savefig(f'./plot/plot/baseline3_s{scenario_number}.png')
...
# Save to CSV
df.to_csv(f'./plot/cost/baseline3_cost_all{scenario_number}.csv', index=False)  # `index=False` to avoid adding row indices
